...
const AddProductPage: React.FC = () => {
  ...
  const [productData, setProductData] = useState({
    ...
    rating: 0,
    reviewCount: 0,
    deliveryTime: '90min', // Defaulted and non-editable
    tags: [] as string[],
    productAmount: 0,
    expiryDate: '',
    prescriptionRequired: false, // Defaulted to false, UI removed
  ...
  // --- Consistent styling for all text inputs with an ALWAYS-ON THICK BLUE BORDER ---
  ...
  const handleInputChange = (
    ...
    // For number fields, ensure only numeric values are processed
  ...
  const handleSubmit = async (e: React.FormEvent<HTMLFormElement>) => {
    ...
    try {
      const productDataToSave = {
        ...
        originalPrice: parseFloat(productData.originalPrice.toString() || '0'),
        productAmount: parseInt(productData.productAmount.toString() || '0', 10),
        prescriptionRequired: false, // Always set to false
      };
      ...
      setProductData({
        ...
        reviewCount: 0, deliveryTime: '90min', tags: [], productAmount: 0,
        expiryDate: '', prescriptionRequired: false, pharmacyName: userData?.pharmacyInfo?.name || '',
      });
      setImagePreviewUrl('');
          ...
          <div className="grid grid-cols-1 md:grid-cols-2 gap-6">
            <div>
              ...
              <input
                ...
                disabled className={disabledInputStyle.replace("mt-1 ", "")} // removed mt-1 for consistency
              />
            </div>
