...
const OrderPrescriptionPage: React.FC = () => {
  ...
  const handleSubmit = async (event: React.FormEvent<HTMLFormElement>) => {
    ...
    const orderData = {
      ...
      userAddressMapLink: selectedAddress.mapLink, // Saving the map link here
      uploadedImageLink: uploadedImageUrl, // Saving the data URL (Base64)
      ...
      medicines: matchingProducts.map(product => ({
        id: product.id,
        name: product.name,
        image: product.image,
        price: product.price,
    ...
    try {
      const userOrdersCollectionRef = collection(db, 'users', user.uid, 'OBP');
      await setDoc(doc(userOrdersCollectionRef), orderData);
