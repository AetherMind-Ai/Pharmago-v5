...
const PharmacyDashboard: React.FC = () => {
    ...
    const [profileViews, setProfileViews] = useState<number>(0);  // State to hold profile views
    const [numberOfProducts, setNumberOfProducts] = useState<number>(0); // State to hold number of products
        ...
        const fetchNumberOfProducts = async () => {
            if (user && userData?.pharmacyInfo?.pharmacyId) {
                try {
                    ...
                    // Assuming products are linked by pharmacyId which is stored in userData.pharmacyInfo
                    const q = query(productsRef, where('pharmacyId', '==', userData.pharmacyInfo.pharmacyId));
                    const querySnapshot = await getDocs(q);
                    setNumberOfProducts(querySnapshot.size);
                } catch (error) {
                    console.error('Error fetching number of products:', error);
                    setNumberOfProducts(0); // Set to 0 on error
                }
            }
        };
    ...
    const recentOrders = [
        ...
        { id: 'ORD-004', customer: 'Yousef Ayman', date: '2025-10-25', amount: 210.00, status: 'Shipped' },
                ...
                <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-4 gap-6 mb-8">
                    <div className="bg-white p-6 rounded-xl shadow-md flex items-center justify-between">
                        <div>
                            ...
                            <p className="text-3xl font-bold text-slate-800">EGP 0.00</p>
                        </div>
                    ...
                    <div className="bg-white p-6 rounded-xl shadow-md flex items-center justify-between">
                        <div>
                            <p className="text-sm font-medium text-gray-500">Num of Products</p>
